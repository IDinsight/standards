...
import { applyOperations, inspectPath, readProjectFile, sameDirectory, verifyNormalDirectory } from './install-files.js';

const packageRoot = fileURLToPath(new URL('../', import.meta.url));
const MARKER = '<!-- standards:framework-owned -->';
const START = '<!-- standards:start -->';
const END = '<!-- standards:end -->';
const CLIENT_PATHS = { codex: '.agents/skills', claude: '.claude/skills' };
...
function jsonObject(text, relative) {
  let value;
...
function blockRange(text, relative) {
...
export async function installProject({ projectRoot, clients = ['codex', 'claude'], mode = null }) {
  ...
  if ((await readdir(projectRoot)).some((name) => name.startsWith('.standards-install-'))) {
    throw new Error('Interrupted STANDARDS installer transaction found; inspect its backups before retrying');
  }
  ...
  for (const client of allClients) {
    for (const role of assets.roles) {
      await queueDirectory(`${CLIENT_PATHS[client]}/${role}`, sourcePath(`skills/${role}`));
    }
  }
  if (allClients.includes('claude')) {
    ...
    if (prepared.manifestChanged || !runtime) {
  ...
  const changed = await applyOperations(projectRoot, operations);
  return { action: runtime ? (changed ? 'Updated' : 'Verified') : 'Installed',
    version: assets.version, mode: projectMode, clients: allClients, changed, warnings };
}
